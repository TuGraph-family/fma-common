...
FMA_UNIT_TEST(RotatingFiles) {
    ...
    std::string dir = "binlogs";
    {
        ...
        RotatingFiles rf(dir, std::move(fgen), max_file_size);
        {
            ...
            FMA_CHECK_EQ(files.size(), (n_writes + write_per_file - 1) / write_per_file + 1);
            ...
            for (size_t i = 0; i < files.size(); i++) {
                ...
                FMA_CHECK_EQ(in.Size(), buf_size);
                std::string buf(in.Size(), 0);
                FMA_CHECK_EQ(in.Read(&buf[0], in.Size()), in.Size());
                for (size_t w = 0; w < write_per_file; w++) {
                    FMA_CHECK_EQ(buf.substr(w * byte_per_write, byte_per_write),
        ...
        FMA_CHECK_EQ(rf.ListFiles().size(), 1);
        {
            for (size_t i = 0; i < n_writes; i++) {
                rf.Write(std::string(byte_per_write, (char)(i % 128)));
            }
